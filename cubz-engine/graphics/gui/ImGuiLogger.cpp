...
namespace cubz::graphics::gui {
    ...
    void ImGuiLogger::log(const cubz::utility::LogEntry &entry) {
        addLog("[%s] %s\n", cubz::utility::logLevelToString(entry.level).c_str(), entry.message.c_str());
    }
    ...
    void ImGuiLogger::render(const char *title, bool *p_open) {
